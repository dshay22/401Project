...
void deal_card_to_player(card deck[108], card **head, card **tail, int *indexLastCard); // deals a card from the deck array into a players hand

bool can_card_be_played(card deck[108], card cardPlayed, int *turnCount,
                        char changedColor[7]); // checks if a card can be played

bool play_a_card(card deck[108], card **head, card **tail, card *discarded, int pickedCard, int currentTotalCards, int *turnCount, char changedColor[7]); // deletes a card from the players hand as they play
