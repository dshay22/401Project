// This project was made by DORIAN SHAY and SEBASTIAN LONGORIA
//
//
...
int main(void) {
  ...
  while (playAgain == 'y') {
    ...
    card *headCard1 = '\0';
    card *headCard2 =
        '\0'; // these two lines create a head for each player hand and are user
              // to make a list of each player hand

    card *tailCard1 =
        '\0'; // these two lines create a tail for each player hand and are user
              // to make a list of each player hand
    card *tailCard2 = '\0';
    ...
    printf(
        "Press 1 to shuffle the UNO deck or 2 to load a deck from a file: \n");
    scanf(" %d", &userShuffle); // Checks what players want to do;

    if ((userShuffle != 1) && (userShuffle != 2)) {
      while ((userShuffle != 1) && (userShuffle != 2)) {
        printf("Press 1 to shuffle the UNO deck or 2 to load a deck from a "
  ...
  //   deck[index].value = '\0';
  //   deck[index].action[0] = '\0'; // After last card is dealt that card spot in
  //                                 // the array becomes '\0'.
  ...
  //   } else if ((strcmp(deck[0].color, "Black") == 0) &&
  //              (deck[0].value == -1)) { // Printing last played card
  //     printf("Discard Pile: %s\n", deck[0].action);
  //   } else if ((strcmp(deck[0].color, "Black") != 0) && (deck[0].value == -1)) {
  //     printf("Discard Pile: %s %s\n", deck[0].color, deck[0].action);
  //   }

  //   while (totalCardsPlayerHand1 != 0 ||
  //          totalCardsPlayerHand2 !=
  //              0) { // condition to check the total cards in each player hand,
  //                   // if zero a player has won loop ends.

  //     if (canPlayer1Play ==
  //         true) { // condition to check if player 1 can play because no skip or
  //                 // draw card was played then
  //       canPlayer2Play = true;
  //       printf("Player 1 hand: ");
  //       IntNode_PrintNodeData(
  //           headCard1); // prints the linked list for player 1 hand
  //       printf("\n");
  //       while (
  //           x !=
  //           true) { // x is false to start and in line 119 x is set to true if a
  //                   // card can be played this condition checks if the card
  //                   // selected by a player is valid within the rules of Uno

  //         printf("Select a card to play (1-%d) or press 0 to draw dards\n",
  //                totalCardsPlayerHand1);
  //         scanf("%d", &targetCard); // asks and scans the card 1-7 of player
  //                                   // hand

  //         if (targetCard == 0) { // if player has no playable cards then
  //                                // pressing zero means they can draw a card
  //           deal_card_to_player(
  //               deck, &headCard1, &tailCard1,
  //               &index); // call function to draw a single card from the deck
  //           printf("Player 1 hand: ");
  //           IntNode_PrintNodeData(headCard1); // after card is drawn this
  //                                             // reprints the hand for redability
  //           printf("\n");
  //           totalCardsPlayerHand1 +=
  //               1; // counter to ensure the hand total is correct in this case
  //                  // the total increases by one
  //         } else {
  //           x = play_a_card(
  //               deck, &headCard1, &tailCard1, &discarded, targetCard,
  //               &totalCardsPlayerHand1, &turnCount,
  //               &changedColor); // function to check is card can be played
  //         }
  //       }
  ...
  //         printf("Discard Pile: %s %d\n", discarded->color,
  //                discarded->value); // discarded as mentioned abouve is the card
  //                                   // a player played
  //       } // these ifstaments print the value of discarded
  ...
  //         printf(
  //             "Player one what color do you choose? (Blue, Red, Green, "
  ...
  //                                                                // staments
  //                                                                // check
  //         scanf("%s", changedColor); // if these statments detrmine a special
  //                                    // card has been played then action happen
  //         canPlayer2Play = true;
  //         printf("Player chose the new color %s\n",
  //                changedColor); // if wild is played then the person can changeg
  //                               // the color
  ...
  //         for (int i = 0; i < 4; i++) {
  //           deal_card_to_player(deck, &headCard2, &tailCard2,
  //                               &index); // deals four cards to other player
  //         }
  //         canPlayer2Play = false; // skips otherplayers turn
  //         totalCardsPlayerHand2 =
  //             totalCardsPlayerHand2 + 4; // increments the total by 4
  //       } else if (strcmp(discarded->action, "Draw-Two") == 0) {
  //         for (int i = 0; i < 2; i++) {

  //           deal_card_to_player(deck, &headCard2, &tailCard2,
  //                               &index); // draw two card for the other player.
  //         }
  //         canPlayer2Play = false; // skips other player turn by setting there
  //                                 // condition to false
  //         totalCardsPlayerHand2 = totalCardsPlayerHand2 + 2;
  //       } else if (strcmp(discarded->action, "Skip") == 0) {
  //         canPlayer2Play =
  //             false; // skips other players turn by setting condition to false
  //       } else if (strcmp(discarded->action, "Reverse") == 0) {
  //         canPlayer2Play = false;
  //       } else if (strcmp(discarded->action, "Play") ==
  //                  0) { // a regular card was played so the other player is
  //                       // allowed to play and the condition is true
  //         canPlayer2Play = true;
  //       }
  //       ++turnCount; // turncount is incrimented
  //       deck[turnCount].value =
  //           discarded->value; // the deack is now reassigned where the empty
  //                             // space is to the discarded card
  ...
  //     if (canPlayer2Play ==
  //         true) { // checks conditions of the if statments above
  //       canPlayer1Play = true;
  ...
  //         } else {
  //           x = play_a_card(
  //               deck, &headCard2, &tailCard2, &discarded, targetCard,
  //               &totalCardsPlayerHand2, &turnCount,
  //               &changedColor); // x iss set to true if the card can be played
  //         }
  //       }

  //       x = false;
  //       totalCardsPlayerHand2 -=
  //           1; // total cards is decreimented if a card is played.
  ...
  //     }
  //   }
  //   // free(headCard1);                                      //after a player
  //   // has won everything is freed free(headCard2); free(tailCard1);
  //   // free(tailCard2);
  //   // free(deck);
